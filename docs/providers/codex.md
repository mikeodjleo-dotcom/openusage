...
OpenUsage also discovers additional file-backed Codex homes kept as dot-directories in your home folder (for example `~/.codex-cli2`) or directly under `~/.config`. Each distinct ChatGPT account becomes its own Codex card, with credential refresh and local session scanning pinned to that home. Two homes carrying the same account are folded into one card, so copied or aliased homes do not inflate the account count. A discovered home must contain OAuth credentials that identify the account; API-key-only or unidentifiable files are ignored. Extra cards can be renamed from their context menu or Customize screen.
...
- **An extra account does not appear** — keep that login in its own Codex home with an `auth.json` that contains ChatGPT OAuth tokens and an account identity. Homes nested more deeply than `~/.config/<name>` are intentionally not scanned.
