...
5. The existing release workflow builds, Developer-ID signs, notarizes, and publishes the tagged DMG and Sparkle appcast.
...
The fork must use `main` as its maintained branch, enable GitHub Pages for the `gh-pages` branch, and provide the signing secrets already consumed by `.github/workflows/release.yml`: `APPLE_CERTIFICATE`, `APPLE_CERTIFICATE_PASSWORD`, `APPLE_ID`, `APPLE_PASSWORD`, `APPLE_TEAM_ID`, `SPARKLE_PRIVATE_KEY`, and `SPARKLE_PUBLIC_KEY`. The iCloud profile secrets are not used for Aix tags.
